...
public interface MensagemRepository extends JpaRepository<Mensagem, Integer> {
    ...
    @Query("select new eventify.api_spring.dto.ChatListaDto(m.buffet.id, m.buffet.nome, m.mensagem, m.data) from Mensagem m where m.usuario.id = :idUsuario order by m.id desc limit 1")
